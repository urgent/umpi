...
/*
==============================
Resido_Recent_Listing
==============================
*/

function recent_property_widget_func()
{
    register_widget('Resido_Recent_Listing');
}
add_action('widgets_init', 'recent_property_widget_func', 2);

class Resido_Recent_Listing extends WP_Widget
{

    public function __construct()
    {
        $widget_ops = array('classname' => 'recent_property_widget', 'description' => __("Latest CPT's & Posts."));
        parent::__construct('wpsites-recent-posts', __('Resido Recent Listings'), $widget_ops);
        $this->alt_option_name = 'recent_property_widget';
    ...
    public function widget($args, $instance)
    {
        $cache = array();
        if (!$this->is_preview()) {
            $cache = wp_cache_get('resido_recent_property', 'widget');
        }
        ...
        $title = (!empty($instance['title'])) ? $instance['title'] : __('Recent Property');
...
?>
        ...
        if (!$this->is_preview()) {
            ...
            wp_cache_set('resido_recent_property', $cache, 'widget');
        } else {
            ob_end_flush();
        }
    }
    ...
    public function update($new_instance, $old_instance)
    {
        ...
        if (isset($alloptions['recent_property_widget']))
            delete_option('recent_property_widget');
    ...
    {
        wp_cache_delete('resido_recent_property', 'widget');
    }

    public function form($instance)
    {
        $title     = isset($instance['title']) ? esc_attr($instance['title']) : 'Recent Property';
