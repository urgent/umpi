...
abstract class mtekk_adminKit
{
	...
	function version_check($version)
	{
		...
		if ($version && version_compare($version, $this::version, '<') && is_array($this->opt)) {
			...
			$this->messages[] = new mtekk_adminKit_message(
				esc_html__('Your settings are for an older version of this plugin and need to be migrated.', $this->identifier, 'Notice')
					. $this->admin_anchor('upgrade', __('Migrate the settings now.', $this->identifier), __('Migrate now.', $this->identifier)),
				'warning'
			);
		...
		// Do a quick version check
		elseif ($version && version_compare($version, $this::version, '>') && is_array($this->opt)) {
			// Let the user know that their settings are for a newer version
			$this->messages[] = new mtekk_adminKit_message(
				esc_html__('Your settings are for a newer version of this plugin.', $this->identifier, 'warning')
					. $this->admin_anchor('upgrade', __('Migrate the settings now.', $this->identifier), __('Attempt back migration now.', $this->identifier)),
				'warning'
			);
			// Output any messages that there may be
			$this->messages();
			return true;
		} elseif (!is_array($this->opt)) {
			// Throw an error since it appears the options were never registered
			$this->messages[] = new mtekk_adminKit_message(
				esc_html__('Your plugin install is incomplete.', $this->identifier, 'warning')
					. $this->admin_anchor('upgrade', __('Load default settings now.', $this->identifier), __('Complete now.', $this->identifier)),
				'error'
			);
			// Output any messages that there may be
			$this->messages();
			return false;
		} elseif (!$this->opts_validate($this->opt)) {
			// Throw an error since it appears the options contain invalid data
			$this->messages[] = new mtekk_adminKit_message(
				esc_html__('One or more of your plugin settings are invalid.', $this->identifier)
					. $this->admin_anchor('fix', __('Attempt to fix settings now.', $this->identifier), __('Fix now.', $this->identifier)),
				'error'
			);
			// Output any messages that there may be
			$this->messages();
	...
	protected function opts_update_loop(&$opts, $input)
	{
		...
		foreach ($opts as $option => $value) {
			...
			elseif (isset($input[$option]) || $option[0] == 'b') {
				switch ($option[0]) {
	...
	static function parse_args($args, $defaults = '')
	{
		...
		if (is_array($defaults)) {
			return self::array_merge_recursive($defaults, $r);
		}
		return $r;
	}
	...
	static function array_merge_recursive($arg1, $arg2)
	{
		foreach ($arg2 as $key => $value) {
			if (array_key_exists($key, $arg1) && is_array($value)) {
				$arg1[$key] = self::array_merge_recursive($arg1[$key], $value);
			} else {
				$arg1[$key] = $value;
			}
		}
	...
	function opts_update()
	{
		...
		if ($updated && count(array_diff_key($input, $this->opt)) == 0) {
			...
			$this->messages[] = new mtekk_adminKit_message(
				esc_html__('Settings successfully saved.', $this->identifier)
					. $this->admin_anchor('undo', __('Undo the options save.', $this->identifier), __('Undo', $this->identifier)),
				'success'
			);
		} elseif (!$updated && count(array_diff_key($opt_prev, $this->opt)) == 0) {
			...
			$this->messages[] = new mtekk_adminKit_message(
				esc_html__('Some settings were not saved.', $this->identifier)
					. $this->admin_anchor('undo', __('Undo the options save.', $this->identifier), __('Undo', $this->identifier)),
				'warning'
			);
			...
			$this->messages[] = new mtekk_adminKit_message($temp . '<br />' . sprintf(esc_html__('Please include this message in your %1$sbug report%2$s.', $this->identifier), '<a title="' . sprintf(esc_attr__('Go to the %s support forum.', $this->identifier), $this->short_name) . '" href="' . $this->support_url . '">', '</a>'), 'info');
		}
	...
	function opts_import()
	{
		...
		if ($dom->load($_FILES[$this->unique_prefix . '_admin_import_file']['tmp_name'])) {
			...
			$this->messages[] = new mtekk_adminKit_message(
				esc_html__('Settings successfully imported from the uploaded file.', $this->identifier)
					. $this->admin_anchor('undo', __('Undo the options import.', $this->identifier), __('Undo', $this->identifier)),
				'success'
			);
		} else {
	...
	function opts_reset()
	{
		...
		$this->messages[] = new mtekk_adminKit_message(
			esc_html__('Settings successfully reset to the default values.', $this->identifier)
				. $this->admin_anchor('undo', __('Undo the options reset.', $this->identifier), __('Undo', $this->identifier)),
			'success'
		);
		add_action('admin_notices', array($this, 'messages'));
	}
	...
	function opts_undo()
	{
		...
		$this->messages[] = new mtekk_adminKit_message(
			esc_html__('Settings successfully undid the last operation.', $this->identifier)
				. $this->admin_anchor('undo', __('Undo the last undo operation.', $this->identifier), __('Undo', $this->identifier)),
			'success'
		);
		add_action('admin_notices', array($this, 'messages'));
	}
	 ...
	 * @param object $option
	 */
	public static function get_valid_id($option)
	{
	...
	{
		$opt_id   = self::get_valid_id($option);
...
?>
		<tr valign="top">
			...
			<td>
				...
				if ($description !== '') {
				?>
					<p class="description"><?php printf($description); ?></p><?php } ?>
			</td>
		</tr>
	<?php
	}
