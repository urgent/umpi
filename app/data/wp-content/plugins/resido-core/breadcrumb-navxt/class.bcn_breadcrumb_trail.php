...
class bcn_breadcrumb_trail
{
	...
	protected function post_hierarchy($id, $type, $parent = null)
	{
		...
		if ($this->opt['bpost_' . $type . '_hierarchy_display']) {
			...
			elseif ($this->opt['Spost_' . $type . '_hierarchy_type'] === 'BCN_POST_PARENT') {
				if ($parent == null) {
	...
	public function fill()
	{
		...
		elseif (is_archive()) {
			...
			elseif (is_post_type_archive() && !isset($type->taxonomy) && (!is_numeric($this->opt['apost_' . $type_str . '_root']) || $this->opt['bpost_' . $type_str . '_archive_display'])) {
		...
		elseif (is_404()) {
			...
			elseif ($this->opt['bblog_display'] || is_home()) {
				$type_str = 'post';
			}
